...
ifdef::env-github[]
Source: link:../../main/java/io/aeron/samples/tutorial/archive/TutorialPublisherOne.java#L16-L28[Connect Aeron & Archive]
endif::[]
ifndef::env-github[]
...
ifdef::env-github[]
Source: link:../../main/java/io/aeron/samples/tutorial/archive/TutorialPublisherOne.java#L33-L61[Publish Messages]
endif::[]
ifndef::env-github[]
[source,java,indent=0]
----
...
subscriber in order to establish a connection.  IPC also requires at least one subscriber to support a flow of events,
but will buffer messages until term is full rather than reject messages immediately.
...
As the log indicates this will have published and archived 10 messages.
...
separate node to the one recording the publication.  Given our subscriber is only going retrieve data, we don't need to
...
ifdef::env-github[]
Source: link:../../main/java/io/aeron/samples/tutorial/archive/TutorialSubscriberOne.java#L16-L28[Connect Aeron & Archive]
endif::[]
ifndef::env-github[]
[source,java,indent=0]
----
...
one which allows fetching of all of the recordings available within an Archive within a range of recording ids.  It
is also possible to query for recordings that partially match a channel definition or by recordingId.

This will list recordings in the `Archive` from recordingId `0`.  The second parameter is the number of recordings that
will be returned.  This can be used to page through the recordings if required.  The third parameter is a callback that
will receive the meta information about the archive (link to descriptor).

We will implement a simple handler for the recording descriptors that will, for each closed recording, replay all its
messages.

ifdef::env-github[]
Source: link:../../main/java/io/aeron/samples/tutorial/archive/TutorialSubscriberOne.java#L43-L76[Replay Messages]
endif::[]
ifndef::env-github[]
[source,java,indent=0]
----
