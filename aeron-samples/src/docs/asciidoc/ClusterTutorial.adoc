...
Aeron Cluster uses an implementation of the https://raft.github.io/[Raft Consensus Algorithm] and an event-sourced model
for persistence to provide a framework for high-performance fault-tolerant services.  Systems build using Aeron Cluster
are fully linearizable consistency model and can cope with partial system failure as long as a quorum of `⌊n/2⌋ + 1`
...
Aeron Cluster's persistence model is designed around the concept of event sourcing.  Martin Fowler
...
For a service, given a known initial state and a reproducible sequence of events can be restored to any state that it
held in the past.  This provides a means to restore a system that has stopped (either crashed or manually stopped) or
create a copies of that state on other nodes.
...
same.  Ensuring that the application logic is deterministic is outside of the control of Aeron Cluster and is the
responsibility of the developer(s) building the application.

[TIP]
====
Be careful to ensure determinism in event-sourced services, common pitfalls include:
...
that must be implementation by the application that receive all of the appropriate event from cluster.
...
production code would like like this, you would probably want to have a cleaner separation between concerns.

First we must define the link between the application logic and Aeron Cluster.  This is done implementing the
...
the snapshot is loaded.  We also take a reference to the cluster at this point as we will need it in the future.
...
<1> The snapshot can be null (this occurs the first time that the service is started).

=== Handling Messages

The `onSessionMessage` callback is the main entry point for requests coming into the cluster.  Messages reach here by
being published to the cluster's ingress channel.  This method will also be passed messages from the log when it is
recovering from the log.  It is from within this method we will want to interact with our application logic.  Cluster
provides a reliable timestamp to this method.  As mentioned earlier this is one of the challenges of building event
sourced systems. Use this value as the timestamp within your application state and it will be consistent under replay.
...
for `offer` is check as it is a non-blocking call and not guaranteed to succeed.
...
snapshot is written, we can look at how it is loaded.  The `Image` provided to the `onStart` has a method that will
indicate if there is no more data available.  However, application code should also encode enough information that the
end of the stream can be detect from the store messages.  The the two approaches can be used to do sanity checking that
...
each node within the cluster needs to bind to a number of ports, so we need to make sure are no clashes.  We could do
this with VMs or containers, but in the interest of simplicity we are going to specify a port range for each node.  I.e:
...
<4> Create a shutdown barrier that will be used later to cleanly trap exit signals and allow the service to exit.
...
First off we need a way to connect our client to the cluster.  As you may of guess all of the message between the client
and the cluster happens via Aeron, but we have an additional layer call AeronCluster that handles connections to the
independent nodes and provides methods for offering messages into the cluster and polling for responses.
...
viable to launch a single media driver and have multiple clients on the same host sharing the same media driver.
...
response, which will limit responses to the session that sent them.  We are adjusting the port per client to allow for
multiple clients on the same machine with multiple media drivers.  If a single media driver was used then this would not
be necessary.
...
----
Sent (7079011707660026522,132,107)
SessionMessage(11,7079011707660026522,155,12422,false)
Sent (7079011707660026523,132,12429)
SessionMessage(11,7079011707660026523,132,12429,true)
Sent (7079011707660026524,132,12429)
SessionMessage(11,7079011707660026524,132,12429,false)
Sent (7079011707660026525,132,12435)
SessionMessage(11,7079011707660026525,132,12435,true)
Sent (7079011707660026526,132,12439)
SessionMessage(11,7079011707660026526,132,12439,true)
Sent (7079011707660026527,132,12446)
SessionMessage(11,7079011707660026527,132,12446,true)
Sent (7079011707660026528,132,12449)
SessionMessage(11,7079011707660026528,132,12449,true)
Sent (7079011707660026529,132,12458)
SessionMessage(11,7079011707660026529,132,12458,true)
Sent (7079011707660026530,132,12462)
SessionMessage(11,7079011707660026530,132,12462,true)
Sent (7079011707660026531,132,12463)
SessionMessage(11,7079011707660026531,132,12463,true)
----
...
[subs = "attributes+"]
----
java -cp <AERON_HOME>/build/libs/aeron-all-{aeronVersion}.jar /dev/shm/aeron-${USER}-0/consensus-module list-members
----
...
[subs = "attributes+"]
----
java -cp <AERON_HOME>/build/libs/aeron-all-{aeronVersion}.jar io.aeron.cluster.ClusterTool \
  /dev/shm/aeron-${USER}-<leaderMemberId>/consensus-module pid
----
...
This will print out a simple number which is the pid of the leader process.
...
java -cp <AERON_HOME>/build/libs/aeron-all-{aeronVersion}.jar \
  /dev/shm/aeron-${USER}-0/consensus-module list-members
----
