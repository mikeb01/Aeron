...
to have some functionality that is stateful, rather than something like an echo service.
...
deserializing the data from the image and initialize the state of the service.
...
<2> Use `Cluster::idle(int)` to allow the service to pause in a friendly manner when the image is not yet complete but
not data was received on the last poll.

=== onTakeSnapshot

Rather than jump ahead to message handling, to complete the picture around start up we should look at how the data makes
its way into the snapshot.  There is a callback `ClusterService::onTakeSnapshot(ExclusivePublication)` that will be
called when it is time to snapshot the state of the service.
