...
import static io.aeron.samples.SampleConfiguration.MESSAGE_LENGTH;
 ...
 */
public class RandomSizePublisher
{
    ...
    public static void main(final String[] args) throws InterruptedException
    {
        ...
        {
            final UnsafeBuffer buffer = new UnsafeBuffer(BufferUtil.allocateDirectAligned(MESSAGE_LENGTH, 64));
            buffer.setMemory(0, buffer.capacity(), (byte)'z');
            final Random r = new Random(0);

            for (long i = 0; i < NUMBER_OF_MESSAGES; i++)
            {
                final int length = r.nextInt(MESSAGE_LENGTH - 1) + 1;
                System.out.print("Offering " + i + "/" + NUMBER_OF_MESSAGES + " (" + length + ") - ");
